...
for i in range(ITERATIONS):
    for threshold in thresholds:
        ...
        evaluation = evaluator.evaluate(merged_ranges = chromatin_algos.algo5d_aggregated(n_clusters=n_clusters, threshold=threshold), model_name=f"k-means 5 features n_clusters={n_clusters}, threshold={threshold}")
        result = pd.concat([result, evaluation], axis=1)
        result.to_csv(FILEPATH, sep = '\t')
...
for i in range(ITERATIONS):
    for threshold in thresholds:
        ...
        evaluation = evaluator.evaluate(merged_ranges = chromatin_algos.algo_prototypes_aggregated(n_clusters=n_clusters, threshold=threshold), model_name=f"k-prototypes 5 features n_clusters={n_clusters}, threshold={threshold}")
        result = pd.concat([result, evaluation], axis=1)
        result.to_csv(FILEPATH, sep = '\t')
